...
- 2025年3月-4月：各参赛队伍进行模型微调及提示词设计（组织方可为各参赛队伍提供技术指导，指导方式与时间待定）；
- 2025年4月中旬：组织方发送测试集；
...
经讨论，本次评测将区分“微调”与“不微调”两个测试方向。微调方向允许利用样例集数据对模型进行微调或蒸馏（样例集数据将于近期更新）；不微调方向则不允许利用样例集数据对模型进行微调或蒸馏，参赛队伍只能通过优化提示词设计来获取回答。两个测试方向将分开评奖，参赛队伍既可以选择参加其中一个方向的测试，也可以同时选择参加两个方向的测试。无论选择哪个测试方向，所有测试过程均需在评测报告中详细说明。
