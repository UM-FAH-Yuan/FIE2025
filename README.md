...
- 2025年2月上旬：组织方发布正式测试集（组织方届时将集中召开一次任务说明会）；
- 2025年2月- 4月：各参赛队伍开展评测（组织方将为各参赛队伍提供技术指导，指导方式与时间待定）；
- 2025年4月30日：参赛队伍提交中文叙实性推理评测任务技术报告（中文或英文），用于审稿；
...
- 参赛队伍需要自行选定若干大型语言模型（型号不限）；利用测试集数据编制提示词，并以API访问的方式逐条发送给被试模型；要求模型以"text"字段值为依据来判断"hypothesis"字段值的真值情况，记录模型的返回结果；最终将结果整理为json格式的数据文件。
- 真值情况包括三种：
...
我们以编号为"NatS_001"的真实语料样例集中的数据为例。
...
本届评测将设置一、二、三等奖，奖项按总正确率从高到低颁发（奖金待定）。其中，一等奖0-1名，二等奖0-2名，三等奖0-3名。荣誉证书将由中国中文信息学会颁发。
...
[1]袁毓林.(2021).从语言的“多声性”看“假装”句的解读歧异. 语言战略研究(05),77-90. doi:10.19689/j.cnki.cn10-1361/h.20210506.

[2]袁毓林.(2020).“忘记”类动词的叙实性漂移及其概念结构基础. 中国语文(05),515-526+638. doi:CNKI:SUN:YWZG.0.2020-05-001.

[3]袁毓林.(2020).叙实性和事实性：语言推理的两种导航机制. 语文研究(01),1-9. doi:CNKI:SUN:YWYJ.0.2020-01-001.

[4]袁毓林.(2020).“记得”的叙实性漂移及其概念结构基础. 语言教学与研究(01),36-47. doi:CNKI:SUN:YYJX.0.2020-01-007.

[5]李新良 & 袁毓林.(2017).“知道”的叙实性及其置信度变异的语法环境. 中国语文(01),42-52+127. doi:CNKI:SUN:YWZG.0.2017-01-003.

[6]李新良 & 袁毓林.(2016).反叙实动词宾语真假的语法条件及其概念动因. 当代语言学(02),194-215. doi:CNKI:SUN:DDYX.0.2016-02-004.
